...
strCurDir = filesys.GetParentFolderName(strCurDir) 'Because this is saved in a subfolder now
...
adoconn.Open "Driver={MySQL ODBC 8.1 ANSI Driver};Server=" & DBLocation & ";" & _
			"Database=software_matrix; User=" & DBUser & "; Password=" & DBPass & ";"


TopTenUpdate 'Report on top 10 most needed updates by device and app
...
if outputl <> "" then
	...
	SendMail RptToEmail, "Software Matrix: Top 10 Most Needed Updates Report"
	outputl = ""
end if
...
Function TopTenUpdate()
	'Top 10 by Computer
	str = "SELECT Computer, trim(group_concat(' ', Name, ' (', Version_Newest, ')')) 'Applications', count(Name) 'Need Updates' from (SELECT A.ID, A.Computer, A.Name, A.Version, D.Version_Newest FROM software_matrix.applicationsdump as A inner join software_matrix.discoveredapplications as D on A.Name = D.Name where A.Version <> D.Version_Newest) as NeedsUpdate group by Computer order by count(Name) DESC LIMIT 10;"
	rs.Open str, adoconn, 2, 1 'OpenType, LockType
	if not rs.eof then
		'Header Info
		outputl = outputl & "<p><b>Most Needed Updates by Computer:</b></p>" & vbcrlf
		outputl = outputl & "<table>" & vbcrlf
		outputl = outputl & "<tr>" & vbcrlf
		outputl = outputl & "  <th>Computer</th>" & vbcrlf
		outputl = outputl & "  <th>Applications</th>" & vbcrlf
		outputl = outputl & "  <th>Need Updates</th>" & vbcrlf
		outputl = outputl & "</tr>" & vbcrlf
	...
	do while not rs.eof
		outputl = outputl & "<tr>" & vbcrlf
		outputl = outputl & "  <td><a href=""" & BaseURL & "/allapps.php?computer=" & rs("Computer") & """>" & rs("Computer") & "</a></td>" & vbcrlf
		outputl = outputl & "  <td>" & rs("Applications") & "</td>" & vbcrlf
		outputl = outputl & "  <td>" & rs("Need Updates") & "</td>" & vbcrlf
		outputl = outputl & "</tr>" & vbcrlf
	...
	if not rs.eof then
		'Header Info
		outputl = outputl & "<p><b>Most Needed Updates by Application:</b></p>" & vbcrlf
		outputl = outputl & "<table>" & vbcrlf
		outputl = outputl & "<tr>" & vbcrlf
		outputl = outputl & "  <th>Name</th>" & vbcrlf
		outputl = outputl & "  <th>Computers</th>" & vbcrlf
		outputl = outputl & "  <th>Need Updates</th>" & vbcrlf
	...
	do while not rs.eof
		outputl = outputl & "<tr>" & vbcrlf
		outputl = outputl & "  <td><a href=""" & BaseURL & "/allapps.php?name=" & rs("Name") & """>" & rs("Name") & "</a></td>" & vbcrlf
